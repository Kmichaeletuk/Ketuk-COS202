...
def show_edit_todo_form(todo_id: int, request: Request, session: SessionDep):
    todo = session.get(Todo, todo_id)
    if not todo:
        raise HTTPException(status_code=404, detail="Todo not found")
...
async def update_todo(todo_id: int, request: Request, session: SessionDep):
    ...
    request.session["flash"] = "Todo updated successfully!" # Flash message
    return RedirectResponse(url='/', status_code=303)
...
def delete_todo(todo_id: int, request: Request, session: SessionDep):
    ...
    request.session["flash"] = "Todo deleted successfully!" # Flash message
    return RedirectResponse(url='/', status_code=303)
